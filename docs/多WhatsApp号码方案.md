可以支持，而且结合现在的调度中心/路由中心，建议分两层推进。

## 当前实现状态（2026-04-02）

Phase 1 已落地为“一个租户可拥有多个 WhatsApp 渠道实例”，并补了以下约束：

1. `channel_configs` 不再限制 `(tenant_id, channel_type)` 唯一。
2. WhatsApp `phoneNumberId` 通过“仅对启用实例生效”的唯一索引约束，且唯一性判断与运行时入站匹配保持同一套数字归一化规则。
3. `web`、`webhook` 仍是单例自动补齐。
4. `whatsapp` 支持多实例，但当租户没有任何 WhatsApp 实例时，系统会自动补一个未绑定占位实例，保证租户始终能看到 WhatsApp 渠道入口。
5. 未绑定的 WhatsApp 实例可以删除，但前提是它从未被任何历史 `conversations.channel_id` 引用。
6. 一旦某个实例已经承载过历史会话，即使解绑号码，也只能停用，不能物理删除，避免历史会话变成悬挂渠道。

在这个基础上，再决定第二阶段是否要把调度规则细化到 `channelId`。
