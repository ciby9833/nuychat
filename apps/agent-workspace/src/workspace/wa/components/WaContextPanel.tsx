  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#111b21] text-[#e9edef]">
      <div className="border-b border-[#222d34] bg-[#202c33] px-4 py-3">
        <div className="text-[15px] font-medium text-white">会话信息</div>
      </div>

      <div className="min-h-0 flex-1 overflow-auto">
        <div className="flex flex-col items-center border-b border-[#222d34] px-6 py-8 text-center">
          ...
          <div className="mt-4 text-[22px] font-medium text-white">{title}</div>
          <div className="mt-1 text-sm text-[#8696a0]">{subtitle}</div>
          <div className="mt-2 text-xs text-[#8696a0]">
            {detail?.conversation.conversationType === "group" ? `${detail?.members.length ?? 0} 位成员` : "私聊"}
          </div>
        </div>

        <div className="border-b border-[#222d34] px-5 py-5">
          <div className="text-xs font-medium uppercase tracking-[0.08em] text-[#8696a0]">当前回复人</div>
          <div className="mt-3 text-[18px] font-medium text-white">{detail?.conversation.currentReplierName || "未接管"}</div>
          <div className="mt-1 text-sm text-[#8696a0]">{detail?.permissions.canReply ? "你可以回复" : "你当前仅可查看"}</div>
        </div>

        {detail?.permissions.canForceAssign ? (
          <div className="border-b border-[#222d34] px-5 py-5">
            <div className="text-xs font-medium uppercase tracking-[0.08em] text-[#8696a0]">主管分配</div>
            <div className="mt-3 space-y-2">
              {session.memberships.map((membership) => (
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-[10px] border border-[#2a3942] bg-[#202c33] px-3 py-2 text-left text-xs text-[#d1d7db] hover:bg-[#2a3942] disabled:opacity-50"
                >
        ...
        <div className="px-5 py-5">
          <div className="text-xs font-medium uppercase tracking-[0.08em] text-[#8696a0]">成员</div>
          <div className="mt-3 space-y-2">
            {detail?.members.map((member) => (
              <div key={member.memberRowId} className="rounded-[10px] border border-[#2a3942] bg-[#202c33] px-3 py-2 text-xs text-[#aebac1]">
                <div className="font-medium text-[#e9edef]">{member.displayName || member.participantJid}</div>
                <div className="mt-1 text-[#8696a0]">{member.isAdmin ? "管理员" : "成员"}</div>
              </div>
            ))}
